    ...
    if (!chat && info_input.text.length() > 0) {
        ...
        for (int n = 0; n < command_list.size(); n++) {
            ...
            if (command.length() > 0) {
                ...
                if (dice_command.size() == 2 && string_stuff.is_number(dice_command[0]) &&
                    string_stuff.is_number(dice_command[1])) {

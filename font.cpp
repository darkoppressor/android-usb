...
void Bitmap_Font::show (double x, double y, string text, string font_color, double opacity, double scale_x,
                        double scale_y, double angle, SDL_Rect allowed_area) {
    // Temporary offsets.
    double X = x, Y = y;
    ...
    for (short show = 0; text[show] != '\0'; show++) {
        ...
        if (text[show] != '\xA') {
            if (X + get_letter_width() * scale_x >= 0 && X <= main_window.SCREEN_WIDTH &&
                Y + get_letter_height() * scale_y >= 0 && Y <= main_window.SCREEN_HEIGHT) {
                ...
                if (!allowed_area_present ||
                    ...
                    if (shadow_distance != 0 && engine_interface.option_font_shadows) {
                        // Render the shadow.
                        render_sprite((int) X + shadow_distance, (int) Y + shadow_distance,
                                      *image.get_image(sprite.name), &chars[ascii], opacity, scale_x, scale_y, angle,
                                      "ui_black");
                    }

                    // Render the character.
                    render_sprite((int) X, (int) Y, *image.get_image(sprite.name), &chars[ascii], opacity, scale_x,
                                  scale_y, angle, font_color);
                }
            }

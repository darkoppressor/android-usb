    ...
    while (!load->eof()) {
        ...
        // Rgb
            ...
            if (rgb_values.size() == 3) {
                colors[colors.size() - 1].set_rgb(string_stuff.string_to_long(rgb_values[0]),
                                                  string_stuff.string_to_long(rgb_values[1]),
                                                  string_stuff.string_to_long(rgb_values[2]));
            }
        }
...
void Engine_Interface::load_information (File_IO_Load* load) {
    ...
    while (!load->eof()) {
        ...
        // Location
            ...
            windows[windows.size() - 1].informations[windows[windows.size() - 1].informations.size() -
                                                     1].x = string_stuff.string_to_long(location[0]);
            windows[windows.size() - 1].informations[windows[windows.size() - 1].informations.size() -
                                                     1].y = string_stuff.string_to_long(location[1]);

            windows[windows.size() - 1].informations[windows[windows.size() - 1].informations.size() -
                                                     1].start_x = windows[windows.size() -
                                                                          1].informations[windows[windows.size() -
                                                                                                  1].informations.size()
                                                                                          - 1].x;
            windows[windows.size() - 1].informations[windows[windows.size() - 1].informations.size() -
                                                     1].start_y = windows[windows.size() -
                                                                          1].informations[windows[windows.size() -
                                                                                                  1].informations.size()
                                                                                          - 1].y;
        }
        // Text
            ...
            windows[windows.size() - 1].informations[windows[windows.size() - 1].informations.size() -
                                                     1].text = string_stuff.process_newlines(line);
        }
        // Tooltip text
            ...
            windows[windows.size() - 1].informations[windows[windows.size() - 1].informations.size() -
                                                     1].tooltip_text = string_stuff.process_newlines(line);
        }
        // Text mutable
            ...
            windows[windows.size() - 1].informations[windows[windows.size() - 1].informations.size() -
                                                     1].text_mutable = string_stuff.string_to_bool(line);
        }
        // Max text length
            ...
            windows[windows.size() - 1].informations[windows[windows.size() - 1].informations.size() -
                                                     1].max_text_length = string_stuff.string_to_long(line);
        }
        // Allowed input
        ...
        // Scrolling
            ...
            windows[windows.size() - 1].informations[windows[windows.size() - 1].informations.size() -
                                                     1].scrolling = string_stuff.string_to_bool(line);
        }
        // Scroll dimensions
            ...
            windows[windows.size() - 1].informations[windows[windows.size() - 1].informations.size() -
                                                     1].scroll_width =
                string_stuff.string_to_long(scroll_dimensions[0]);
            windows[windows.size() - 1].informations[windows[windows.size() - 1].informations.size() -
                                                     1].scroll_height =
                string_stuff.string_to_long(scroll_dimensions[1]);
        }
        // Sprite
        else if (!multi_line_comment && boost::algorithm::starts_with(line, str_sprite)) {
            // Clear the data name.
        ...
        // Background opacity
            ...
            windows[windows.size() - 1].informations[windows[windows.size() - 1].informations.size() -
                                                     1].background_opacity = string_stuff.string_to_double(line);
        }
        // Font color
        else if (!multi_line_comment && boost::algorithm::starts_with(line, str_font_color)) {
            // Clear the data name.
        ...
        // If the line ends the information.
            ...
            windows[windows.size() - 1].informations[windows[windows.size() - 1].informations.size() -
                                                     1].set_dimensions();
            windows[windows.size() - 1].informations[windows[windows.size() - 1].informations.size() -
                                                     1].center_in_window(windows[windows.size() - 1].w,
                                                                         windows[windows.size() - 1].h);
...
void Engine_Interface::load_button (File_IO_Load* load) {
    ...
    while (!load->eof()) {
        ...
        // Location
            ...
            windows[windows.size() - 1].buttons[windows[windows.size() - 1].buttons.size() -
                                                1].x = string_stuff.string_to_long(location[0]);
            windows[windows.size() - 1].buttons[windows[windows.size() - 1].buttons.size() -
                                                1].y = string_stuff.string_to_long(location[1]);
            ...
            windows[windows.size() - 1].buttons[windows[windows.size() - 1].buttons.size() -
                                                1].start_x = windows[windows.size() -
                                                                     1].buttons[windows[windows.size() -
                                                                                        1].buttons.size() - 1].x;
            windows[windows.size() - 1].buttons[windows[windows.size() - 1].buttons.size() -
                                                1].start_y = windows[windows.size() -
                                                                     1].buttons[windows[windows.size() -
        ...
        if (top_window != 0) {
            ...
            for (int i = 0; i < top_window->informations.size(); i++) {
                if (top_window->informations[i].text_mutable || top_window->informations[i].scrolling) {
                    ...
                    if (top_window->get_scrolling_button_count() > 0 &&
                        top_window->informations[i].y >= scrolling_buttons_offset) {
                        real_y += scrolling_buttons_offset + top_window->get_scrolling_button_count() - 1;
                    }
    ...
    if (mutable_info_selected()) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            if ((ptr_mutable_info->allows_input("letters") &&
                ...
                if (!(text[i] == '`' && (gui_mode == "mouse" || gui_mode == "keyboard") &&
...
void Engine_Interface::prepare_for_input () {
    ...
    for (int i = 0; i < closed_windows.size() && window_under_mouse == 0; i++) {
        Collision_Rect box_a(mouse_x, mouse_y, cursor_width, cursor_height);
        Collision_Rect box_b(windows[closed_windows[i]].x, windows[closed_windows[i]].y, windows[closed_windows[i]].w,
                             windows[closed_windows[i]].h);
        ...
        case SDL_FINGERDOWN:
            ...
            if (!event_consumed) {
                vector<SDL_GameControllerButton> touch_buttons =
                    touch_controller.check_for_button_press(event.tfinger.x, event.tfinger.y);
    ...
    if (configure_command != -1) {
        ...
        switch (event.type) {
            case SDL_CONTROLLERBUTTONDOWN:
                ...
                if (!event_consumed &&
                    (event.cbutton.button == SDL_CONTROLLER_BUTTON_START || allow_keys_and_buttons)) {
            ...
            case SDL_KEYDOWN:
                ...
                if (!event_consumed && event.key.repeat == 0 &&
                    (event.key.keysym.scancode == SDL_SCANCODE_ESCAPE ||
                     event.key.keysym.scancode == SDL_SCANCODE_AC_BACK || allow_keys_and_buttons)) {
                    if (event.key.keysym.scancode != SDL_SCANCODE_ESCAPE &&
                        event.key.keysym.scancode != SDL_SCANCODE_AC_BACK &&
                        event.key.keysym.scancode != SDL_SCANCODE_MENU) {
                        if (configure_command < game_commands.size()) {
    ...
    switch (event.type) {
        case SDL_CONTROLLERDEVICEADDED:
            ...
            if (SDL_IsGameController(event.cdevice.which)) {
                ...
                if (controller != 0) {
                    ...
                    if (SDL_JoystickIsHaptic(SDL_GameControllerGetJoystick(controller))) {
                        ...
                        if (controller_object->haptic != 0) {
                            if (SDL_HapticRumbleSupported(controller_object->haptic)) {
                                if (SDL_HapticRumbleInit(controller_object->haptic) != 0) {
                                    string msg = "Error initializing rumble for haptic on controller " +
                                                 string_stuff.num_to_string(event.cdevice.which) + ": ";
                            ...
                            string msg = "Error opening haptic for controller " +
                                         string_stuff.num_to_string(event.cdevice.which) + ": ";
                ...
                message_log.add_error("Joystick \"" + joystick_name +
        ...
        case SDL_CONTROLLERAXISMOTION:
            ...
            if (!event_ignore_command_set) {
                ...
                if (!event_consumed && is_any_window_open() && !console.on && !mutable_info_selected() &&
                    gui_axis_nav_last_direction != "up" && event.caxis.axis == SDL_CONTROLLER_AXIS_LEFTY &&
                    event.caxis.value < -controller_dead_zone) {
                ...
                if (!event_consumed && is_any_window_open() && !console.on && !mutable_info_selected() &&
                    gui_axis_nav_last_direction != "down" && event.caxis.axis == SDL_CONTROLLER_AXIS_LEFTY &&
                    event.caxis.value > controller_dead_zone) {
        ...
        case SDL_CONTROLLERBUTTONDOWN:
            ...
            if (!event_ignore_command_set) {
                if (mutable_info_selected() && !allow_screen_keyboard()) {
                    if (!controller_text_entry_small) {
                        if (!event_consumed && ptr_mutable_info->allows_input("newline") &&
                            ptr_mutable_info->text.length() < ptr_mutable_info->max_text_length &&
                        ...
                        if (!event_consumed && is_console_selected() &&
                            event.cbutton.button == SDL_CONTROLLER_BUTTON_LEFTSTICK) {
                            console.do_command();

                            event_consumed = true;
                        } else if (!event_consumed && is_chat_selected() &&
                                   event.cbutton.button == SDL_CONTROLLER_BUTTON_LEFTSTICK) {
                            chat.send_chat();
                        ...
                        if (!event_consumed && ptr_mutable_info->allows_input("backspace") &&
                            event.cbutton.button == SDL_CONTROLLER_BUTTON_LEFTSHOULDER &&
                            ptr_mutable_info->text.length() > 0) {
                            input_backspace();
                        ...
                        if (!event_consumed && event.cbutton.button == SDL_CONTROLLER_BUTTON_A) {
                            ...
                            if ((int) text_entry_char[0] == 1) {
                                if (ptr_mutable_info->allows_input("backspace") &&
                                    ptr_mutable_info->text.length() > 0) {
                                    input_backspace();
                                }
                ...
                if (!event_consumed && (mutable_info_selected() || is_any_window_open()) &&
                    event.cbutton.button == SDL_CONTROLLER_BUTTON_B) {
                    gui_nav_back_controller();
                ...
                if (!event_consumed && is_any_window_open() && !console.on &&
        ...
        case SDL_KEYDOWN:
            ...
            if (!event_ignore_command_set) {
                if (mutable_info_selected()) {
                    ...
                    if (!event_consumed && event.key.repeat == 0 && is_console_selected() &&
                        (event.key.keysym.scancode == SDL_SCANCODE_RETURN ||
                         event.key.keysym.scancode == SDL_SCANCODE_KP_ENTER)) {
                        console.do_command();

                        event_consumed = true;
                    } else if (!event_consumed && event.key.repeat == 0 && is_chat_selected() &&
                               (event.key.keysym.scancode == SDL_SCANCODE_RETURN ||
                                event.key.keysym.scancode == SDL_SCANCODE_KP_ENTER)) {
                        chat.send_chat();
                    ...
                    if (!event_consumed && ptr_mutable_info->allows_input("backspace") &&
                        event.key.keysym.scancode == SDL_SCANCODE_BACKSPACE && ptr_mutable_info->text.length() > 0) {
                        input_backspace();
                ...
                if (event.key.repeat == 0) {
                    ...
                    if (!event_consumed && (is_any_window_open() || console.on || chat.on) &&
...
void Engine_Interface::handle_input_states () {
    ...
    if (axis_left_x >= -controller_dead_zone && axis_left_x <= controller_dead_zone &&
        axis_left_y >= -controller_dead_zone && axis_left_y <= controller_dead_zone) {
        gui_axis_nav_last_direction = "none";
    }
...
string Engine_Interface::get_system_info () {
    ...
    if (engine_interface.controllers.size() > 0) {
        ...
        for (int i = 0; i < engine_interface.controllers.size(); i++) {
            if (SDL_GameControllerGetAttached(engine_interface.controllers[i].controller)) {
                ...
                if (engine_interface.controllers[i].haptic != 0 &&
                    SDL_HapticOpened(SDL_HapticIndex(engine_interface.controllers[i].haptic))) {
                    haptic = true;
                }
    ...
    msg += "Resolution (Logical): " + string_stuff.num_to_string(logical_width) + " x " +
           string_stuff.num_to_string(logical_height) + "\n";
    msg += "Logical Viewport: " + string_stuff.num_to_string(rect.x) + "," + string_stuff.num_to_string(rect.y) + "," +
           string_stuff.num_to_string(rect.w) + "," + string_stuff.num_to_string(rect.h) + "\n";
    msg += "Render Scale: " + string_stuff.num_to_string(scale_x) + "," + string_stuff.num_to_string(scale_y) + "\n";
    msg += "Resolution (Actual): " + string_stuff.num_to_string(actual_width) + " x " +
           string_stuff.num_to_string(actual_height) + "\n";
    msg += "Renderer: " + renderer_name + "\n";
    msg += "Max Texture Size: " + string_stuff.num_to_string(info.max_texture_width) + " x " +
           string_stuff.num_to_string(info.max_texture_height) + "\n";
    msg += "Current Gui Mode: " + string_stuff.first_letter_capital(engine_interface.gui_mode) + "\n";

    msg += "Mouse Position (Logical): " + string_stuff.num_to_string(mouse_x) + "," +
           string_stuff.num_to_string(mouse_y) + "\n";
    msg += "Mouse Position (Actual): " + string_stuff.num_to_string(mouse_real_x) + "," +
           string_stuff.num_to_string(mouse_real_y) + "\n";
...
void Engine_Interface::render_gui_selector () {
    ...
    if (object_pos.x != -1) {
        if (gui_selector_style == "standard" || gui_selector_style == "corners" || gui_selector_style == "chasers") {
            ...
            render_rectangle_empty(object_pos.x - offset, object_pos.y - offset, object_pos.w + offset * 2.0,
                                   object_pos.h + offset * 2.0, 1.0, current_color_theme()->gui_selector_1, thickness);
        }
        ...
        if (gui_selector_style == "corners") {
            ...
            render_rectangle(object_pos.x - corner_size / 2.0 + 1, object_pos.y - corner_size / 2.0 + 1, corner_size,
                             corner_size, 1.0, current_color_theme()->gui_selector_2);
            render_rectangle(object_pos.x + object_pos.w - corner_size / 2.0 - 1, object_pos.y - corner_size / 2.0 + 1,
                             corner_size, corner_size, 1.0, current_color_theme()->gui_selector_2);
            render_rectangle(object_pos.x - corner_size / 2.0 + 1, object_pos.y + object_pos.h - corner_size / 2.0 - 1,
                             corner_size, corner_size, 1.0, current_color_theme()->gui_selector_2);
            render_rectangle(object_pos.x + object_pos.w - corner_size / 2.0 - 1,
                             object_pos.y + object_pos.h - corner_size / 2.0 - 1, corner_size, corner_size, 1.0,
                             current_color_theme()->gui_selector_2);
        }
        ...
        if (gui_selector_style == "underline") {
            ...
            for (int i = 0; i < thickness; i++) {
                render_line(object_pos.x, object_pos.y + object_pos.h + i, object_pos.x + object_pos.w - 1,
                            object_pos.y + object_pos.h + i, 1.0, current_color_theme()->gui_selector_1);
            }
        }
        ...
        if (gui_selector_style == "chasers") {
            ...
            for (int i = 0; i < gui_selector_chasers.size(); i++) {
                render_rectangle(gui_selector_chasers[i].x - chaser_size / 2.0,
...
void Engine_Interface::render_small_text_inputter () {
    render_rectangle(0, 0, main_window.SCREEN_WIDTH, main_window.SCREEN_HEIGHT, 0.75,
                     current_color_theme()->window_border);
    render_rectangle(window_border_thickness, window_border_thickness,
                     main_window.SCREEN_WIDTH - window_border_thickness * 2.0,
                     main_window.SCREEN_HEIGHT - window_border_thickness * 2.0, 0.75,
                     current_color_theme()->window_background);

    if (mutable_info_selected()) {
        Bitmap_Font* font = get_font("small");

        font->show(window_border_thickness + 2.0, window_border_thickness + 2.0, ptr_mutable_info->text,
    ...
    double x_offset = ((double) main_window.SCREEN_WIDTH - font->get_letter_width() * 13.0) / 2.0 +
    ...
    text_selector.render(x_offset + text_entry_small_selector.x * font->get_letter_width() -
                         (text_selector.get_width() - font->get_letter_width()) / 2.0,
                         buttons_start_y + text_entry_small_selector.y * font->get_letter_height() -
                         (text_selector.get_height() - font->get_letter_height()) / 2.0);
}
...
void Engine_Interface::render_text_inputter () {
    ...
    render_rectangle(x_adjust, 0, main_window.SCREEN_WIDTH / 2, main_window.SCREEN_HEIGHT, 0.75,
                     current_color_theme()->window_background);
    render_rectangle_empty(x_adjust, 0, main_window.SCREEN_WIDTH / 2, main_window.SCREEN_HEIGHT, 0.75,
    ...
    font->show(x_adjust + (main_window.SCREEN_WIDTH / 2 - text.length() * font->spacing_x) / 2,
               outer_center_y - 8.0 + font->spacing_y, text, "ui_white");

    text = "[LB] Backspace";
    font->show(x_adjust + 100, words_y, text, current_color_theme()->window_font);
    text = "Space [RB]";
    font->show(x_adjust + main_window.SCREEN_WIDTH / 2 - text.length() * font->spacing_x - 100, words_y, text,
               current_color_theme()->window_font);

    text = "(LT) Caps";
    font->show(x_adjust + 100, words_y + 30, text, current_color_theme()->window_font);
    text = "Numbers (RT)";
    font->show(x_adjust + main_window.SCREEN_WIDTH / 2 - text.length() * font->spacing_x - 100, words_y + 30, text,
               current_color_theme()->window_font);

    if (selected_chunk == -1) {
        if (is_console_selected()) {
            text = "(A) Auto-complete";
            font->show(x_adjust + (main_window.SCREEN_WIDTH / 2 - text.length() * font->spacing_x) / 2, words_y + 60,
                       text, current_color_theme()->window_font);
        }

        text = "(B) Back/Done";
        font->show(x_adjust + (main_window.SCREEN_WIDTH / 2 - text.length() * font->spacing_x) / 2, words_y + 90, text,
                   current_color_theme()->window_font);
    }

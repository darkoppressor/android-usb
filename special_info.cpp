    ...
    if (special_info.length() > 0) {
        if (special_info == "configure_command") {
            if (engine_interface.configure_command != -1 &&
                engine_interface.configure_command < engine_interface.game_commands.size()) {
                text += "Inputs currently bound to \"" +
                        engine_interface.game_commands[engine_interface.configure_command].title + "\":" + "\n\n";

                const char* ckey =
                    SDL_GetScancodeName(engine_interface.game_commands[engine_interface.configure_command].key);
                const char* cbutton =
                    SDL_GameControllerGetStringForButton(
                        engine_interface.game_commands[engine_interface.configure_command].button);
                const char* caxis =
                    SDL_GameControllerGetStringForAxis(
                        engine_interface.game_commands[engine_interface.configure_command].axis);
                ...
                if (allow_keys_and_buttons) {
                    ...
                    if (ckey != 0 &&
                        engine_interface.game_commands[engine_interface.configure_command].key !=

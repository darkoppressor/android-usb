...
Version_Series::Version_Series (int get_major_1, int get_minor_1, int get_micro_1, int get_major_2, int get_minor_2,
                                int get_micro_2) {
    major_1 = get_major_1;
    minor_1 = get_minor_1;
    micro_1 = get_micro_1;
...
string Engine_Interface::get_version () {
    return string_stuff.num_to_string(Version::MAJOR) + "." + string_stuff.num_to_string(Version::MINOR) + "." +
           string_stuff.num_to_string(Version::MICRO);
}
...
int Engine_Interface::which_version_series (vector<Version_Series>* version_series, int major, int minor, int micro) {
    for (int i = 0; i < version_series->size(); i++) {
        if (version_compare(major, minor, micro, version_series->at(i).major_1, version_series->at(i).minor_1,
                            version_series->at(i).micro_1) >= 0 && version_compare(major, minor, micro,
                                                                                   version_series->at(i).major_2,
                                                                                   version_series->at(i).minor_2,
                                                                                   version_series->at(i).micro_2) <=
            0) {
            return i;
        }
    }
...
bool Engine_Interface::version_compatible (string name_to_check) {
    ...
    if (which_version_series(&version_series, major, minor, micro) != which_version_series(&version_series,
...
bool Engine_Interface::save_game_commands () {
    ...
    for (int i = 0; i < game_commands.size(); i++) {
        ...
        save << "\t" << game_commands[i].name << "," << game_commands[i].title << "," <<
            string_stuff.add_newlines(game_commands[i].description) << "," <<
            string_stuff.bool_to_string(game_commands[i].dev) << "," << key << "," << button << "," << axis << "\n";
    }
...
bool Engine_Interface::save_servers () {
    ...
    for (int i = 0; i < network.server_list.size(); i++) {
        save << "\t" << network.server_list[i].name << "," << network.server_list[i].address << "," <<
            network.server_list[i].port << "," << network.server_list[i].password << "\n";
    }
        ...
        while (!load.eof()) {
            ...
            // Load data based on the line.
                ...
                if (command_parts.size() == 4) {
                    ...
                    network.server_list[network.server_list.size() -
                                        1].port = string_stuff.string_to_unsigned_long(command_parts[2]);

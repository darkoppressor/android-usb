...
bool Game::handle_game_command_gui (string command_name) {
    ...
    if (command_name == "help") {
        ...
        if (!notice->on) {
            string help =
                "Scroll the file list up and down using the mouse scroll wheel, the arrow keys, or a gamepad.\n";

            help +=
                "Click on a file/directory by clicking on it with the left mouse button (or press Enter or A on a gamepad).\n";
            help += "Clicking on a directory navigates to it.\n\n";
            help +=
                "Drag files/directories onto the program window to copy them from your computer to the current directory.\n\n";
            help +=
                "Hold Control (or the Left Shoulder button on a gamepad) when clicking on a file/directory to select it.\n";
            help +=
                "Press A (or the Right Shoulder button on a gamepad) to select all files/directories in the current directory.\n";
            ...
            help += "Files copied to your computer are located in '" + engine_interface.get_home_directory() +
                    "files/'\n";
            help +=
                "WARNING: Copying/moving/renaming files in either direction has no prompt and simply overwrites any identically\nnamed files already located in the same place. Use wisely.\n\n";
    ...
    if (in_progress) {
        for (int i = 0; i < engine_interface.game_commands.size() && !event_consumed; i++) {
            if ((event.type == SDL_CONTROLLERBUTTONDOWN &&
                 engine_interface.game_commands[i].button == event.cbutton.button) ||
                (event.type == SDL_KEYDOWN && event.key.repeat == 0 &&
                 engine_interface.game_commands[i].key == event.key.keysym.scancode)) {

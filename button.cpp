...
void Button::set_dimensions_text () {
    ...
    h = engine_interface.gui_border_thickness * 2.0 + (string_stuff.newline_count(text) + 1) * ptr_font->spacing_y +
        ptr_font->gui_padding_y;
}
...
bool Button::mouse_button_up (Window* parent_window) {
    ...
    if (clicked) {
        if (((engine_interface.gui_mode == "mouse" || engine_interface.gui_mode == "keyboard") &&
             (keystates[SDL_SCANCODE_RCTRL] || keystates[SDL_SCANCODE_LCTRL])) ||
            (engine_interface.gui_mode == "controller" && engine_interface.controller_state(-1,
                                                                                            SDL_CONTROLLER_BUTTON_LEFTSHOULDER)))
        {
            window_opened_on_top = fire_alt_event1(parent_window);
        } else if (((engine_interface.gui_mode == "mouse" || engine_interface.gui_mode == "keyboard") &&
    ...
    if (!moused_over && !clicked) {
        ...
        if (sprite.name.length() > 0) {
            ...
            if (engine_interface.current_color_theme()->button_border != "<INVISIBLE>") {
                render_rectangle(x_offset + x, y_offset + y, w, h, 1.0,
                                 engine_interface.current_color_theme()->button_border);
            }
            ...
            if (engine_interface.current_color_theme()->button_background != "<INVISIBLE>") {
                render_rectangle(x_offset + x + engine_interface.gui_border_thickness,
                                 ...
                                 h - engine_interface.gui_border_thickness * 2.0, 1.0,
                                 engine_interface.current_color_theme()->button_background);
            }

            // Display the button's text.
            if (font_color_real != "<INVISIBLE>") {
                ptr_font->show(x_offset + x + engine_interface.gui_border_thickness,
                               y_offset + y + engine_interface.gui_border_thickness, text, font_color_real);
            }
        }
    }
    // Moused over.
        ...
        if (sprite_moused.name.length() > 0) {
            ...
            if (engine_interface.current_color_theme()->button_background_moused != "<INVISIBLE>") {
                render_rectangle(x_offset + x + engine_interface.gui_border_thickness,
                                 ...
                                 w - engine_interface.gui_border_thickness * 2.0,
                                 h - engine_interface.gui_border_thickness * 2.0, 1.0,
                                 engine_interface.current_color_theme()->button_background_moused);
            }
        ...
        if (sprite_click.name.length() > 0) {
            ...
            if (engine_interface.current_color_theme()->button_background_click != "<INVISIBLE>") {
                render_rectangle(x_offset + x + engine_interface.gui_border_thickness,
                                 ...
                                 w - engine_interface.gui_border_thickness * 2.0,
                                 h - engine_interface.gui_border_thickness * 2.0, 1.0,
                                 engine_interface.current_color_theme()->button_background_click);
            }

        ...
        for (int y = 0; y < surface->h; y++) {
            for (int o_x = 0; o_x < scale_x; o_x++) {
                for (int o_y = 0; o_y < scale_y; o_y++) {
                    surface_put_pixel(new_surface, (int) ((scale_x * x) + o_x), (int) ((scale_y * y) + o_y),
                                      surface_get_pixel(surface, x, y));
                }
            }
        }
    }
...
void render_rtt (double x, double y, Rtt_Data rtt_source, double opacity, double scale_x, double scale_y, double angle,
                 string color_name, bool flip_x) {
    SDL_Rect rect_dst;
    ...
    SDL_SetTextureColorMod(rtt_source.texture, color->get_red_short(), color->get_green_short(),
                           color->get_blue_short());

    SDL_RenderCopyEx(main_window.renderer, rtt_source.texture, 0, &rect_dst, -angle, 0, flip);
}

void render_texture (double x, double y, Image_Data image_source, double opacity, double scale_x, double scale_y,
                     double angle, string color_name, bool flip_x) {
    SDL_Rect rect_dst;
    ...
    SDL_SetTextureColorMod(image_source.texture, color->get_red_short(), color->get_green_short(),
                           color->get_blue_short());

    SDL_RenderCopyEx(main_window.renderer, image_source.texture, 0, &rect_dst, -angle, 0, flip);
}

void render_sprite (double x, double y, Image_Data image_source, Collision_Rect* texture_clip, double opacity,
                    double scale_x, double scale_y, double angle, string color_name, bool flip_x) {
    SDL_Rect rect_src;
...
void render_rectangle (double x, double y, double w, double h, double opacity, string color_name) {
    ...
    SDL_SetRenderDrawColor(main_window.renderer, color->get_red_short(), color->get_green_short(),
                           color->get_blue_short(), (short) (opacity * 255.0));
...
void render_rectangle_empty (double x, double y, double w, double h, double opacity, string color_name,
                             double line_width) {
...
void render_circle (double x, double y, double radius, double opacity, string color_name) {
    ...
    render_texture(x - radius, y - radius, *image.get_image("circle"), opacity, sprite_scale, sprite_scale, 0.0,
                   color_name);
}
...
void render_circle_empty (double x, double y, double radius, double opacity, string color_name) {
    ...
    render_texture(x - radius, y - radius, *image.get_image("circle_empty"), opacity, sprite_scale, sprite_scale, 0.0,
                   color_name);
}

...
void Information::set_dimensions () {
    ...
    if (scrolling || (scroll_width > 0 && scroll_height > 0)) {
        ...
        if (text_mutable) {
            ...
            w = engine_interface.gui_border_thickness * 2.0 + string_stuff.longest_line(text) * ptr_font->spacing_x +
                ptr_font->gui_padding_x;
            h = engine_interface.gui_border_thickness * 2.0 + (string_stuff.newline_count(text) + 1) *
                ptr_font->spacing_y + ptr_font->gui_padding_y;
        }
    }
}
...
void Information::scroll_up (short y_offset) {
    if (scrolling) {
        if (y_offset + y + engine_interface.get_font(font)->spacing_y * scroll_offset <
            y_offset + y + h - engine_interface.get_font(font)->spacing_y * 2) {
            scroll_offset += 1;
        }
    }
}
...
void Information::scroll_down (short y_offset) {
    if (scrolling) {
        if (y_offset + y + engine_interface.get_font(font)->spacing_y * scroll_offset +
            engine_interface.get_font(font)->spacing_y * string_stuff.newline_count(text) > y_offset + y) {
            scroll_offset -= 1;
        }
    }
}
...
void Information::handle_input_states (int mouse_x, int mouse_y, short x_offset, short y_offset) {
    if (tooltip_text.length() > 0) {
        ...
        if (engine_interface.mouse_allowed() && engine_interface.gui_mode == "mouse" && collision_check_rect(box_a,
                                                                                                             box_b)) {
            engine_interface.tooltip.setup(tooltip_text, mouse_x, mouse_y);
                   ...
                   engine_interface.is_gui_object_selected(this)) {
...
void Information::render (short x_offset, short y_offset) {
    ...
    if (sprite.name.length() > 0 && background_type == "sprite") {
        ...
        if (engine_interface.current_color_theme()->information_border != "<INVISIBLE>") {
            render_rectangle(x_offset + x, y_offset + y, w, h, background_opacity,
                             engine_interface.current_color_theme()->information_border);
        }

        // Render the background.
        if (engine_interface.current_color_theme()->information_background != "<INVISIBLE>") {
            render_rectangle(x_offset + x + engine_interface.gui_border_thickness,
                             y_offset + y + engine_interface.gui_border_thickness,
                             w - engine_interface.gui_border_thickness * 2.0,
                             h - engine_interface.gui_border_thickness * 2.0, background_opacity,
                             engine_interface.current_color_theme()->information_background);
        }
    }
    ...
    if (text.length() > 0 || text_mutable) {
        ...
        if (font_color_real != "<INVISIBLE>") {
            ptr_font->show(x_offset + x + engine_interface.gui_border_thickness,
                           y_offset + y + engine_interface.gui_border_thickness + scrolling_adjust_y, text,
                           font_color_real, 1.0, 1.0, 1.0, 0.0, allowed_area);

            if (text_mutable && engine_interface.mutable_info_this(this)) {
                ptr_font->show(x_offset + x + engine_interface.gui_border_thickness +
                               (ptr_font->spacing_x * string_stuff.length_of_last_line(text)),
                               y_offset + y + engine_interface.gui_border_thickness + scrolling_adjust_y +
                               (ptr_font->spacing_y * string_stuff.newline_count(text)), "\x7F", font_color_real,
                               engine_interface.cursor_opacity * 0.1, 1.0, 1.0, 0.0, allowed_area);
            }
        }
    }

...
void Network::edit_server (int index, string get_name, string get_address, unsigned short get_port,
                           string get_password) {
    if (index < server_list.size()) {
...
void Network::connect_to_server () {
    if (status == "client") {
        ...
        message_log.add_log("Attempting to connect to server: " + server_address + "|" +
...
void Network::receive_client_list () {
    ...
    for (int i = 0; i < clients_size; i++) {
        ...
        clients.push_back(Client_Data(RakNet::UNASSIGNED_RAKNET_GUID, RakNet::UNASSIGNED_SYSTEM_ADDRESS,
...
void Network::send_input () {
    if (status == "client") {
        if (game.in_progress) {
            if (commands_this_second < rate_commands &&
                ++counter_commands >= (uint32_t) ceil(UPDATE_RATE / (double) rate_commands)) {
                counter_commands = 0;

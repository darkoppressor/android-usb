                ...
                if (valid_commands.size() == 1) {
                    ...
                    if (info_input.text.length() == shared_characters.length() &&
                        ...
                        for (int i = 0; i < valid_commands.size(); i++) {
                            ...
                            if (valid_command_display[valid_command_display.size() - 1].length() + this_line.length() >
                                info_display.w / engine_interface.get_font(info_display.font)->spacing_x) {
                                valid_command_display.push_back("");
                            }
        ...
        switch (event.type) {
            ...
            case SDL_KEYDOWN:
                ...
                if (event.key.repeat == 0) {
                    if ((event.key.keysym.scancode == SDL_SCANCODE_GRAVE && !keystates[SDL_SCANCODE_LSHIFT] &&
                         !keystates[SDL_SCANCODE_RSHIFT]) || event.key.keysym.scancode == SDL_SCANCODE_AC_SEARCH) {

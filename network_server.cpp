        ...
        if (game.in_progress) {
            for (int i = 1; i < clients.size(); i++) {
                if (clients[i].connected) {
                    ...
                    if (clients[i].updates_this_second < client_rate_updates &&
                        ++clients[i].counter_update >= (uint32_t) ceil(UPDATE_RATE / (double) client_rate_updates)) {
...
void Network::send_ping_list () {
    if (status == "server") {
        ...
        stat_counter_bytes_sent += bitstream.GetNumberOfBytesUsed();
        peer->Send(&bitstream, LOW_PRIORITY, RELIABLE_ORDERED, ORDERING_CHANNEL_CHAT, RakNet::UNASSIGNED_RAKNET_GUID,
                   true);
    }
}
...
void Network::send_paused () {
    if (status == "server") {
        ...
        stat_counter_bytes_sent += bitstream.GetNumberOfBytesUsed();
        peer->Send(&bitstream, HIGH_PRIORITY, RELIABLE_ORDERED, ORDERING_CHANNEL_PAUSED, RakNet::UNASSIGNED_RAKNET_GUID,
                   true);
    }
}

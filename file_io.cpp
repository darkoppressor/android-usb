...
string File_IO_Load::get_data () {
    ...
    bool File_IO::is_directory (string path) {
        ...
        if (stat(path.c_str(), &file_info) == 0) {
            ...
            message_log.add_error("Error getting file information for file '" + path + "': " +
                                  string_stuff.num_to_string(errno));
        }
    ...
    bool File_IO_Directory_Iterator_User_Data::is_regular_file () {
        ...
        if (stat(get_full_path().c_str(), &file_info) == 0) {
            ...
            message_log.add_error("Error getting file information for file '" + get_full_path() + "': " +
                                  string_stuff.num_to_string(errno));
        }

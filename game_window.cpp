...
bool Game_Window::initialize_video () {
    ...
    if (engine_interface.option_fullscreen) {
        if (engine_interface.option_fullscreen_mode == "standard") {
            screen = SDL_CreateWindow(engine_interface.game_title.c_str(), SDL_WINDOWPOS_CENTERED,
                                      SDL_WINDOWPOS_CENTERED, 0, 0, flags | SDL_WINDOW_FULLSCREEN_DESKTOP);
        } else if (engine_interface.option_fullscreen_mode == "windowed") {
            screen = SDL_CreateWindow(engine_interface.game_title.c_str(), SDL_WINDOWPOS_CENTERED,
                                      SDL_WINDOWPOS_CENTERED, desired_resolution_x, desired_resolution_y,
...
bool Game_Window::init () {
    ...
    if (Mix_Init(MIX_INIT_OGG) == 0) {
        ...
        if (Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, 2, 1024) == -1) {
            ...
            if (channels_allocated != channels_requested) {
                msg = "Error allocating mixer channels: Requested " + string_stuff.num_to_string(channels_requested) +
                      ", allocated " + string_stuff.num_to_string(channels_allocated) + ".";
                message_log.add_error(msg);
            }
        }
    }
    ...
    for (int i = 0; i < SDL_NumJoysticks(); i++) {
        if (SDL_IsGameController(i)) {
            if (engine_interface.option_accelerometer_controller ||

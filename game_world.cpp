...
File_Data Game_World::check_file (string path) {
    ...
    if (boost::algorithm::contains(run_command(game.option_adb_path + " shell if [ -e \"" + path +
                                               "\" ]; then echo \"yeppers!\"; fi"), "yeppers!")) {
        file_data.exists = true;

        if (boost::algorithm::contains(run_command(game.option_adb_path + " shell if [ -r \"" + path +
                                                   "\" ]; then echo \"yeppers!\"; fi"), "yeppers!")) {
            file_data.is_readable = true;

            if (boost::algorithm::contains(run_command(game.option_adb_path + " shell if [ -h \"" + path +
                                                       "\" ]; then echo \"yeppers!\"; fi"), "yeppers!")) {
                file_data.is_symbolic_link = true;
            }

            if (boost::algorithm::contains(run_command(game.option_adb_path + " shell if [ -d \"" + path +
                                                       "\" ]; then echo \"yeppers!\"; fi"), "yeppers!")) {
                file_data.is_directory = true;
            }

            if (boost::algorithm::contains(run_command(game.option_adb_path + " shell if [ -f \"" + path +
                                                       "\" ]; then echo \"yeppers!\"; fi"), "yeppers!")) {
    ...
    if (starting_path.length() > 0 && file.length() > 0 && file != "." && file != "..") {
        ...
        if (file_data.exists && file_data.is_readable) {
            if (file_data.is_directory) {
                ...
                message_log.add_log("Pulling '" + path + "' to '" + engine_interface.get_home_directory() + "files/" +
                                    cd + "'");
                ...
                run_command(game.option_adb_path + " pull \"" + path + "\" \"" + engine_interface.get_home_directory() +
                            "files/" + cd + file + "\"");

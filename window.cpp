...
void Window::scroll_up () {
    if (y + buttons[last_normal_button + 1].h * scroll_offset <
        y + h - buttons[last_normal_button + 1].h * floor((double) engine_interface.scrolling_buttons_offset / 15.0)) {
        scroll_offset += 1;
    }
}
                ...
                if (button_in_window && buttons[i].is_moused_over(mouse_x, mouse_y, x, button_real_y)) {
                    ...
                    if (buttons[i].has_tooltip()) {
                        ...
                        if (engine_interface.mouse_allowed() && engine_interface.gui_mode == "mouse") {
                            tooltip_x = mouse_x;
                            tooltip_y = mouse_y;
                        } else if (engine_interface.gui_mode == "keyboard" ||
                                   engine_interface.gui_mode == "controller") {
                            int real_y = y;
            ...
            switch (event.type) {
                case SDL_MOUSEBUTTONDOWN:
                    ...
                    if (!event_consumed && event.button.button == SDL_BUTTON_LEFT) {
                        ...
                        Collision_Rect box_a(mouse_x, mouse_y, engine_interface.cursor_width,
                                             engine_interface.cursor_height);
                ...
                case SDL_MOUSEWHEEL:
                    ...
                    if (!event_consumed && event.wheel.y < 0) {
                        if (scrolling_buttons.length() > 0) {
                            Collision_Rect box_a(mouse_x, mouse_y, engine_interface.cursor_width,
                                                 engine_interface.cursor_height);
                            Collision_Rect box_b(x, y + engine_interface.scrolling_buttons_offset, w,
                                                 h - engine_interface.scrolling_buttons_offset - 6);
    ...
    if (on) {
        ...
        // Render the background if it's just a solid color.
            ...
            if (engine_interface.current_color_theme()->window_background != "<INVISIBLE>") {
                render_rectangle(x + engine_interface.window_border_thickness,
                                 y + engine_interface.window_border_thickness,
                                 w - engine_interface.window_border_thickness * 2.0,
                                 h - engine_interface.window_border_thickness * 2.0, 1.0,
                                 engine_interface.current_color_theme()->window_background);
            }
        }

        // Render the title bar.
        if (engine_interface.current_color_theme()->window_title_bar != "<INVISIBLE>") {
            render_rectangle(x + engine_interface.window_border_thickness + 2,
                             y + engine_interface.window_border_thickness + 2,
                             w - engine_interface.window_border_thickness * 2.0 - 4, ptr_font->spacing_y, 1.0,
                             engine_interface.current_color_theme()->window_title_bar);
        }

        ...
        for (packet = peer->Receive(); packet != 0; peer->DeallocatePacket(packet), packet = peer->Receive()) {
            ...
            switch (packet_id) {
                ...
                case ID_CONNECTION_ATTEMPT_FAILED:
                    ...
                    if (status == "client") {
                        message_log.add_log("Could not connect to server: " + server_address + "|" +
                                            string_stuff.num_to_string(server_port));
                ...
                case ID_ALREADY_CONNECTED:
                    ...
                    if (status == "client") {
                        message_log.add_log("Already connected to server: " +
                ...
                case ID_DISCONNECTION_NOTIFICATION:
                    ...
                    if (status == "server") {
                        ...
                        message_log.add_log("Disconnected from server: " +
                                            string(packet->systemAddress.ToString(true)));
                ...
                case ID_CONNECTION_LOST:
                    ...
                    if (status == "server") {
                        ...
                        message_log.add_log("Lost connection to server: " +
                                            string(packet->systemAddress.ToString(true)));
                ...
                case ID_INVALID_PASSWORD:
                    ...
                    if (status == "client") {
                        message_log.add_log("Invalid password for server: " +
                                            string(packet->systemAddress.ToString(true)));
                ...
                case ID_INCOMPATIBLE_PROTOCOL_VERSION:
                    message_log.add_log("Incompatible protocol version: " +
                                        string(packet->systemAddress.ToString(true)) + "\nOur protocol: " +
                                        string_stuff.num_to_string((int) RAKNET_PROTOCOL_VERSION) +
                ...
                case ID_IP_RECENTLY_CONNECTED:
                    ...
                    if (status == "client") {
                        message_log.add_log("Connected too recently to server: " +
                                            string(packet->systemAddress.ToString(true)));
                ...
                default:
                    message_log.add_log("Received message with unknown identifier: " +
                                        string_stuff.num_to_string((int) packet_id));
                    break;
            }
...
string Network::get_stats () {
    ...
    if (status != "off") {
        str_stats += "In: " + string_stuff.num_to_string((long double) stat_bytes_received / (long double) 1000.0) +
                     " kB/s\n";
        str_stats += "Out: " + string_stuff.num_to_string((long double) stat_bytes_sent / (long double) 1000.0) +
                     " kB/s\n";
    }

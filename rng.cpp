...
uint32_t RNG::weighted_random_range(uint32_t lownum,uint32_t highnum,uint32_t target,string weight){
    uint32_t weight_num=RNG_WEIGHT_NORMAL;

    if(weight=="weak"){
        weight_num=RNG_WEIGHT_WEAK;
    }
    else if(weight=="normal"){
        weight_num=RNG_WEIGHT_NORMAL;
    }
    else if(weight=="strong"){
        weight_num=RNG_WEIGHT_STRONG;
    }
    else if(weight=="very_strong"){
        weight_num=RNG_WEIGHT_VERY_STRONG;
    }

    ...
    if (on && message.length() > 0) {
        ...
        if (engine_interface.current_color_theme()->tooltip_background != "<INVISIBLE>") {
            render_rectangle(x + engine_interface.gui_border_thickness, y + engine_interface.gui_border_thickness,
                             ...
                             engine_interface.current_color_theme()->tooltip_background);
        }

        // Display the message text.
        if (engine_interface.current_color_theme()->tooltip_font != "<INVISIBLE>") {
            ptr_font->show(x + engine_interface.gui_border_thickness, y + engine_interface.gui_border_thickness,
                           message, engine_interface.current_color_theme()->tooltip_font);
        }
    }
}

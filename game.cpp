...
void Game::render_scoreboard () {
    if (display_scoreboard) {
        ...
        font->show(72,
                   (main_window.SCREEN_HEIGHT - (string_stuff.newline_count(name_list) + 1) * font->spacing_y) / 2.0,
                   name_list, "ui_white");
        font->show(168,
                   (main_window.SCREEN_HEIGHT - (string_stuff.newline_count(ping_list) + 1) * font->spacing_y) / 2.0,
                   ping_list, "ui_white");
    }
}
